...
import { getCompanyRegistrationBarChartData } from '../Dashboard.service';

const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl px-4 py-3 text-sm bg-white shadow-lg border border-gray-100">
        <p className="text-emerald-500 font-semibold mb-1">{label}</p>
        <p className="text-gray-800 font-black text-base">
          {payload[0].value}
          <span className="text-gray-400 text-xs font-normal ml-1">companies</span>
        </p>
      </div>
    );
  }
...
export const CompanyRegistrationBarChart = () => {
  ...
  const fetchChartData = async () => {
    try {
      setLoading(true);
      const result = await getCompanyRegistrationBarChartData(filters);
    ...
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold tracking-widest uppercase text-emerald-500">Companies</span>
          </div>
          <h3 className="text-lg font-black text-gray-800">Company Registrations</h3>
        ...
        <div className="flex gap-2">
          <select
            className="px-3 py-2 rounded-xl text-sm font-medium text-gray-600 bg-gray-50 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-300 transition-all"
            value={filters.month}
          ...
          <input
            type="number"
            className="w-24 px-3 py-2 rounded-xl text-sm font-medium text-gray-600 bg-gray-50 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-300 transition-all"
            value={filters.year}
      ...
      <div className="flex gap-3 mb-5">
        <div className="px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100">
          ...
          <p className="text-lg font-black text-emerald-600">{totalCount}</p>
        </div>
        {peakDay && peakDay.count > 0 && (
          <div className="px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100">
            <p className="text-xs text-emerald-400">Peak Day</p>
        ...
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center gap-3">
            <div className="flex gap-1 items-end">
              {[0,1,2,3,4].map(i => (
                <div key={i} className="w-2 rounded bg-emerald-100 animate-pulse"
            ...
            <BarChart data={data} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
              ...
              <Bar dataKey="count" radius={[5, 5, 0, 0]} barSize={18} maxBarSize={30}>
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.count > 0 ? 'url(#company-gradient)' : '#f3f4f6'}
                  />
                ))}
              </Bar>
            </BarChart>
